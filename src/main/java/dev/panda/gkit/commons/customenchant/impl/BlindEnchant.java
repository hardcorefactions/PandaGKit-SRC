...
extends ICustomEnchant {
    public BlindEnchant() {
        super("blind", CustomEnchantProperties.builder().setGlow(true).setDisplayName("&8Blind I").setDescription(Arrays.asList("&6&m---------------------", "&7This enchantment has a chance of 5% to give", "&7your enemy blindness.", "", "&eDrag to your armor to apply", "&6&m---------------------")).setMaterial(Material.BOOK).setData(0).setLevel(1).addCustomProperty("chance", 5.0).addCustomProperty("duration", 5).build(), new PotionEffectType[0]);
    }
    ...
    public double onHit(Player player, Player damager, double damage) {
        ...
        PotionRestorer.builder().effect(PotionEffectType.BLINDNESS, (Integer)this.getProperties().getCustomProperty("duration"), this.getProperties().getLevel()).player(player).build().run();
        return damage;
    }
}
