...
public class DizzyEnchant
extends ICustomEnchant {
    public DizzyEnchant() {
        super("dizzy", CustomEnchantProperties.builder().setGlow(true).setDisplayName("&5Dizzy I").setDescription(Arrays.asList("&6&m---------------------", "&7This enchantment has a chance of 2% to give", "&7your enemy confusion.", "", "&eDrag to your armor to apply", "&6&m---------------------")).setMaterial(Material.BOOK).setData(0).setLevel(1).addCustomProperty("chance", 2.0).addCustomProperty("duration", 4).build(), new PotionEffectType[0]);
    }
    ...
    public double onHit(Player player, Player damager, double damage) {
        ...
        PotionRestorer.builder().effect(PotionEffectType.CONFUSION, (Integer)this.getProperties().getCustomProperty("duration"), this.getProperties().getLevel()).player(player).build().run();
        return damage;
    }
}
